...
def gen_rand(num_of_genes):
    i = random.randint(0,num_of_genes-1)
    j = random.randint(1,num_of_genes)
    while(j<=i):
        j = random.randint(0,num_of_genes)
    return i,j

def swapAcut(a,b,max_num_genes):
    p11,p12 = gen_rand(len(a))
    p21,p22 = gen_rand(len(b))
    a[p11:p12],b[p21:p22] = b[p21:p22], a[p11:p12]
    
    while (len(a) > max_num_genes):
        idx = random.randint(0, len(a)-1)
        a.pop(idx)
    while (len(b) > max_num_genes):
        idx = random.randint(0, len(b)-1)
        b.pop(idx)
    
    return a,b

def varAnd(population, toolbox, cxpb, mutpb,mgcrpb,max_genes):
    ...
    # Apply 2-point crossover for multi-gene version
    for i in range(1, len(offspring), 2):
        if random.random() < mgcrpb:
                       
            offspring[i - 1], offspring[i] = swapAcut(offspring[i - 1], offspring[i],max_genes)
    ...
    for i in range(1, len(offspring), 2):
        if random.random() < cxpb:
            ...
            # for multi-gene versions
            idx1 = random.randint(0, len(offspring[i-1])-1)
            idx2 = random.randint(0, len(offspring[i])-1)         
            
            offspring[i - 1][idx1], offspring[i][idx2] = toolbox.mate(offspring[i - 1][idx1],
                                                          offspring[i][idx2])
            del offspring[i - 1].fitness.values, offspring[i].fitness.values
    ...
    for i in range(len(offspring)):
        if random.random() < mutpb:
            ...
            offspring[i][idx], = toolbox.mutate(offspring[i][idx]) # [idx] for multi-gene version
            del offspring[i].fitness.values

    return offspring


def eaSimple(population, toolbox, cxpb, mutpb, mgcrpb, max_num_of_genes, ngen, stats=None, halloffame=None, verbose=__debug__):
    ...
    for gen in range(1, ngen + 1):
        ...
        offspring = varAnd(offspring, toolbox, cxpb, mutpb,mgcrpb,max_num_of_genes)
