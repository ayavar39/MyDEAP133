...
class PrimitiveSetTyped(object):
    ...
    def addEphemeralConstant(self, name, ephemeral, ret_type):
        ...
        """
        module_gp = globals()
        if (name not in module_gp) or (name in module_gp):
...
# GP Mutations                       #
######################################
def mutUniform(individual, expr, pset): # originally was def mutUniform(individual, expr, pset):
